...
Distribution stays `npm install -g cleancopy-cli` (the unscoped name `cleancopy` is blocked by npm's similarity rule against the dormant `clean-copy`; the installed command is still `cleancopy`); the prebuilt Swift helper ships inside the package. (An all-Swift + Homebrew build was considered and rejected in favor of keeping the engine in TypeScript.)

### The cleanup engine — the heart (`src/engine`)

Pure functions: text in, text out, **no side effects and no macOS/clipboard code**. Keeping the engine isolated makes it safe and easy to test, but it is an internal part of the CLI rather than a published library API — deliberately: an `exports` map for the engine was drafted and dropped in review (semver-bound surface with one consumer). The package does ship `dist/**/*.d.ts`, and the private marketing-site repo exact-pins the package and deep-imports `cleancopy-cli/dist/engine/index.js` (explicit file path — directory imports fail under ESM) as an internal, promised-to-no-one path with full types. If outsiders ever depend on internals, the answer is an `./internal/engine` map with a stability disclaimer, not more hidden paths. The pipeline (one file per step):
...
4. **transform** (`transform.ts`) — tidy each block per its type. Only `prose` and `list` blocks above the confidence threshold (`REFLOW_THRESHOLD`) are reflowed (wrapped lines glued back together; for lists, per item, keeping items separate). Everything else is returned verbatim. Joins additionally pass the **forced-break veto**: a wrap only ever breaks a line because the next word didn't fit, so a break whose next word clearly *would* have fit within the block's demonstrated width (longest line, lone outliers demoted when the rest hug a real edge) is deliberate and never joined — unless the line before it is syntactically open (unclosed bracket, dangling word, trailing comma), the fingerprint of a torn drag-select fragment.

Above the per-block pipeline sit two copy-level fences (evidence spans blocks, so they're judged in `clean()` before classification): **two or more shell-prompt lines** (`user@host dir %` / `user@host:path$`) mark the whole copy as a terminal transcript — all verbatim; a lone prompt line freezes just its block. A line-start **`/*` opens a comment fence** freezing everything through `*/`, across blank lines. And a **stability gate** wraps the whole thing: the pipeline runs once more on its own output, and if anything would change (joins can manufacture new wrap geometry) the copy is returned normalize-only — clean() is idempotent by construction, not by hope.
...
- **The byte-identical mimic is unfixable.** Command output whose lines hug a uniform width and break mid-sentence is indistinguishable from wrapped prose by any content rule — same bytes, different intent. The forced-break veto removes every case where geometry *contradicts* wrapping; the mimic that passes it still joins. Hedged by the confidence threshold, manual mode, and the revert hotkey.
- **Prose that quotes two shell prompts freezes wholesale.** An answer explaining "% vs #" quotes both prompt shapes, trips the ≥2-prompt transcript rule, and the whole copy stays verbatim. One quoted prompt costs only its own block. Freeze-direction; the transcript rule's protection is worth it.
- **Hand-wrapped (semantic-linefeed) markdown reflows less than before.** Authors' deliberate mid-paragraph breaks fall short of the wrap edge, so the veto keeps them. For terminal-wrapped prose — full lines by construction — nothing is lost; md sources copied from an editor stay raggeder. Invisible direction, in-domain win.
